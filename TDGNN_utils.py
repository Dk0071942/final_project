...
### create the adjacency list and adjacency matrix
def edgelist2graph(edge_index, nodenum):
    ...
    adjacency_matrix = nx.adjacency_matrix(nx.from_dict_of_lists(adjlist)).toarray()

    return adjlist, adjacency_matrix



def edge_info(data, args):
    adjlist, adjmatrix = edgelist2graph(data.edge_index, data.x.size(0))
    hop_edge_index, hop_edge_att = tree_decomposition(data.x.size(0), args.tree_layer, adjlist, data.edge_index)
    torch.save(hop_edge_index, data.edge_index_location)
    torch.save(hop_edge_att,  data.edge_att_location)
